...
REM --- إيقاف أي نسخة سابقة من البرنامج قبل التثبيت ---
...
REM --- إنشاء أيقونة الاختصار من شعار التطبيق ---
echo.
echo [+] تجهيز أيقونة التطبيق...
set ICO_PATH=%~dp0logo.ico
powershell -Command "Add-Type -AssemblyName System.Drawing; $src='%~dp0frontend\public\logo.jpeg'; if(!(Test-Path $src)){$src='%~dp0شعار جنان بيز.jpeg'}; $img=[System.Drawing.Image]::FromFile($src); $bmp=New-Object System.Drawing.Bitmap($img,64,64); $ico=[System.Drawing.Icon]::FromHandle($bmp.GetHicon()); $fs=[System.IO.File]::OpenWrite('%ICO_PATH%'); $ico.Save($fs); $fs.Close(); $img.Dispose()" >nul 2>&1
...
powershell -Command "$ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%SHORTCUT_PATH%'); $s.TargetPath = '%TARGET_PATH%'; $s.WorkingDirectory = '%~dp0'; $s.IconLocation = '%ICO_PATH%,0'; $s.Description = 'جنان بيز - حلول الاعمال'; $s.Save()" >nul 2>&1
